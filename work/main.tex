...
\paragraph{Readout.} At the recommended operating point the photon flux through the interaction region is $\sim 5\times 10^{15}$~s$^{-1}$ ($N_\gamma^{(1)} \approx 5\times 10^9$ per 1.11-$\mu$s transit at $z_\text{ref}$, summed over both arms); no photon-counting array operates at such flux, and none is required. The readout is shot-noise-limited beam \emph{deflectometry}: the transverse displacement of each transmitted arm is monitored on a position-sensitive (split or lateral-effect) photodiode, whose shot-noise limit realizes exactly the per-photon pointer statistics assumed above. For coherent-state illumination this identification is exact (section~\ref{sec:decoherence}). How this position-tagged record converts into a velocity field is the subject of section~\ref{sec:bohmian}.
...
Relative centering (two colours) & $\delta x_c \leq 0.6$ nm (integrity); drift-only after calibration & common fiber + common reflective path; $\leq 10''$ wedge spec ($0.33$ nm); differential trim servo; static part in DoG family \\
...
\emph{Beyond detection.} Past the detection tier the estimator remains statistics-limited. The GLS trajectory rms improves from $1.25 \pm 0.84$ mm at $N_a = 10^4$ to $184 \pm 93\,\mu$m --- $44\%$ of the fringe --- at $10^7$ per grid point (figure~\ref{fig:reconstruction_asymptotic}(a)), as $N_a^{-0.3}$ overall, and even at $10^7$ atoms per point the rms exceeds the $3.4\,\mu$m noiseless-pipeline floor by more than fifty times: the residual deviation is record noise through the moment contraction, not a geometric or algorithmic limit. Reallocating the atom budget across planes improves the final rms by only $5$--$8\%$ at fixed total atom number in the additive model; a seed-paired end-to-end comparison gives uniform-to-optimal rms ratios of $1.51 \pm 0.31$ ($N_a = 10^6$) and $1.11 \pm 0.19$ ($10^7$), so uniform allocation is near-optimal. The dipole-residual systematics of section~\ref{sec:dipole_lensing} remain subdominant throughout: at the two-stage budget $\eta_\text{bal}\,\varepsilon_\text{res} = 10^{-6}$ the residual biases the final trajectories by $\leq 1.9\%$ of the fringe through this pipeline. The supporting allocation and scaling studies are given in appendix~\ref{app:budgets_allocation}.
...
The structural picture is therefore: the quantum-coherence requirements of the proposal are met by current cold-atom capabilities; the reconstruction formula is verified numerically against the exact (closed-form) Bohmian field; the back-action budget is quantified and simulation-confirmed; and the measurement programme is deliberately modest: conditional-momentum-field detection is feasible at present-day fluxes within weeks, while quantitative mapping and trajectory-bundle reconstruction are identified as objectives for improved flux and coupling. The obstruction is thereby reduced to two finite budgets: a back-action calibration budget, $\eta_\text{bal}\,\varepsilon_\text{res} \leq 1.4\times 10^{-6}$, achievable by the two-stage in situ scheme of section~\ref{sec:dipole_lensing}, and an atom budget, $2.2\times 10^8$ for five-sigma detection of the field, days to weeks at demonstrated fluxes.
...
\paragraph{Seed-paired end-to-end comparison.} A direct end-to-end test, with uniform and optimized allocations run on identical noise seeds, gives uniform-to-optimal rms ratios of $1.51 \pm 0.31$ at $N_a = 10^6$ and $1.11 \pm 0.19$ at $10^7$ (mean $\pm$ SE over 12 seed pairs) --- consistent with the additive prediction within its (large) uncertainty. The conclusion of section~\ref{sec:resolution_limits} follows: uniform allocation is near-optimal, and reallocation is not a substitute for atom number.
