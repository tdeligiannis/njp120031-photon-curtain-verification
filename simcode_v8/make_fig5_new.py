...
S2=pickle.load(open('sim/stage2_results.pkl','rb'))   # direct-pipeline realizations of the SHIPPED run (same derivation as collect_v8_summary.py);
_r2=np.array([r['rms'] for r in S2['runs']])[:,-1]*1e6  # previously a hard-coded 13817.7 +- 11710.4 um from the official machine's log (harness fix 2026-08-25)
a.errorbar([1e4],[_r2.mean()],yerr=[_r2.std(ddof=1)/np.sqrt(_r2.size)],fmt='s',color='C3',ms=4,capsize=2,label='direct moment pipeline ($N_a{=}10^4$)')
