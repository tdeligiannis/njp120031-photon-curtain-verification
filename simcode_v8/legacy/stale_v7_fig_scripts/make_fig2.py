"""Fig 2: what detuning does and does not purchase. Two-level far-detuned scaling, Config-D optics, single pass.
...
kick=np.sqrt(2/np.e)*(hbar/(M*w))*Psc*D/Gam              # g_a = Psc*Delta/Gamma at fixed Psc
...
a.axhline(sres/w**2, color='k', lw=1.4, ls='--', label=fr'ideal bound $\sigma_\mathrm{{res}}/w^2={sres/w**2:.3f}$')
...
b.loglog(Dnu/1e9, kick, color='#2ca02c', lw=1.8,
         label=r'deterministic dipole kick $\sqrt{2/e}\,(\hbar/Mw)\,P_\mathrm{sc}\Delta/\Gamma$')
...
b.plot(xcross, stoch, 'ko', ms=5); b.annotate(fr'crossover $\approx{xcross:.1f}$ GHz', (xcross, stoch),
...
print(f"ratio={ratio[0]:.3e}  ideal={sres/w**2:.4f}  kick(200 GHz)={np.interp(200e9,Dnu,kick):.3f} m/s  stoch={stoch*1e3:.3f} mm/s  crossover={xcross:.2f} GHz")
